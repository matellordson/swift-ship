...
import * as z from "zod"
import { format } from "date-fns"
import { CheckCircle2, Circle, Loader2, Package, User, Truck, Info } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import {
  Form,
...
export default function PackageTimeline() {
  ...
  return (
    <div className="container mx-auto p-4 max-w-3xl mb-5">
      <h1 className="lg:text-3xl font-bold mb-6 text-2xl">Track Package</h1>
        ...
        <div className="space-y-8">
                    ...
                    <div className="flex-grow">
                      ...
                      {packageData[`stage${index + 1}IsCompleted` as keyof PackageData] && (
                        <p className="text-sm text-muted-foreground">
                          {formatValue(packageData[`stage${index + 1}Location` as keyof PackageData] as string | null)} - {
                            packageData[`stage${index + 1}Time` as keyof PackageData]
                              ? format(new Date(packageData[`stage${index + 1}Time` as keyof PackageData] as string), "PPpp")
                              : 'N/A'
                          }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  <p><strong>Name:</strong> {formatValue(packageData.sender_full_name)}</p>
                  <p><strong>Email:</strong> {formatValue(packageData.sender_email)}</p>
                  <p><strong>Phone:</strong> {formatValue(packageData.sender_phone_number)}</p>
                  <p><strong>Address:</strong> {formatValue(packageData.sender_address)}</p>
                  <p><strong>City:</strong> {formatValue(packageData.sender_city)}</p>
                  <p><strong>Country:</strong> {formatValue(packageData.sender_country)}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  <p><strong>Name:</strong> {formatValue(packageData.receiver_full_name)}</p>
                  <p><strong>Email:</strong> {formatValue(packageData.receiver_email)}</p>
                  <p><strong>Phone:</strong> {formatValue(packageData.receiver_phone_number)}</p>
                  <p><strong>Address:</strong> {formatValue(packageData.receiver_address)}</p>
                  <p><strong>City:</strong> {formatValue(packageData.receiver_city)}</p>
                  <p><strong>Country:</strong> {formatValue(packageData.receiver_country)}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  <p><strong>Type:</strong> {formatValue(packageData.package_type)}</p>
                  <p><strong>Dimensions:</strong> {formatValue(packageData.dimension)}</p>
                  <p><strong>Weight:</strong> {formatValue(packageData.weight)}</p>
                  <p><strong>Description:</strong> {formatValue(packageData.description)}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <p><strong>Delivery Date:</strong> {packageData.delivery_date}</p>
                  <p><strong>Tracking Number:</strong> {packageData.tracking_number}</p>
                </div>
              </CardContent>
            </Card>
          </div>
